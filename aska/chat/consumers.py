...
class PrivateChatConsumer(AsyncWebsocketConsumer):
    ...
    def get_messages(self):
        ...
        serializer = ChatMessageSerializer(messages, context={"user": self.scope["user"]}, many=True)
    ...
    async def connect(self):
        ...
        if int(self.current_user_id) < int(self.other_user_id):
            ...
            self.room_name = f"{self.other_user_id}_{self.current_user_id}"
        self.room_group_name = f"chat_{self.room_name}"
        await self.channel_layer.group_add(self.room_group_name, self.channel_name)
        await self.accept()

        messages = await self.get_messages()
        await self.send(json.dumps({"messages": messages, "initial": True}))
