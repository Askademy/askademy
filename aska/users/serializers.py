...
from .choices import SendCodeTo, GradeLevel
...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    send_to = serializers.ChoiceField(choices=SendCodeTo.choices, label="Send code to")
    ...
    def validate_send_to(self, send_to):
        ...
        if send_to == SendCodeTo.EMAIL_ADDRESS and not user_has_email:
            raise serializers.ValidationError("The user has no email address")
        elif send_to == SendCodeTo.PHONE_NUMBER and not settings.CAN_SEND_SMS:
...
class UserSerializer(serializers.ModelSerializer):
    username = serializers.ReadOnlyField(source="get_full_name")
    subjects = serializers.SlugRelatedField(slug_field="name", many=True, queryset=Subject.objects.all())
    schools = serializers.SerializerMethodField("get_schools")
    level = serializers.ChoiceField(choices=GradeLevel.choices, allow_blank=True)
    ...
    def save(self, **kwargs):
        # Check if 'cover_picture' or 'profile_picture' is missing in the validated data
        if  not self.validated_data.get("cover_picture", None):
            # If missing, set it to the old value
            self.validated_data['cover_picture'] = self.instance.cover_picture

        if  not self.validated_data.get("profile_picture", None):
            # If missing, set it to the old value
            self.validated_data['profile_picture'] = self.instance.profile_picture
